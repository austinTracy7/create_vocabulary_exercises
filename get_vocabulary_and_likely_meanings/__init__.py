...
def find_likely_meanings(row):
    ...
    if row["POS"] in ["NNP","NNS","NN","CD","NNPS"]:
        general_pos = "n"
    elif row["POS"] in ["IN","DT","TO","CC"]:
        general_pos = "functional_word" # not meaningful
    elif row["POS"] in ["PRP","WP","WRB","FW","WDT","PDT"]:
        general_pos = "s"
    elif row["POS"] in ["VBZ","VBP","VB","MD","VBN","VBG","VBD"]:
        general_pos = "v"
    elif row["POS"] in ["RB","RP","RBR"]:
        general_pos = "r"
    elif row["POS"] in ["JJ","JJS"]:
        general_pos = "a"
    else:
        return None
    # selecting the likely meanings
    output = [x for x in wn.synsets(row["Token"]) if str(x).find(f'.{general_pos}.') > -1]
    if output == []:
        output = None
    else:
        return output
